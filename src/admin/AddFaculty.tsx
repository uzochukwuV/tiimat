...
function AddFaculty() {
    ...
    const [facultyDesc, setFacultyDesc] = useState("") as any;
      ...
      const addcoursetofaculty = (e:FormEvent<HTMLFormElement>)=>{
        ...
        addFaculty(facultyName, facultyDesc).then((data)=>{
            console.log(data);
            alert("Faculty Added")
        ...
        <Link to={"/admin/course/edit/"} >Edit course</Link>
         ...
         <h1 className=" -translate-y-8 text-3xl">Add Faculty</h1>
            <form onSubmit={addcoursetofaculty} method="post">
                ...
                <input value={facultyDesc} onChange={updateDesc} type="text" placeholder="Description" className="w-[80vw]  px-6 flex-1 block relative rounded-xl focus:outline-gray-300 ring ring-white  h-14  bg-[var(--background)]" />
