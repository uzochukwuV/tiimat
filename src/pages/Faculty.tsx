  ...
  return (
    <div className=" min-h-screen h-full antialiased">
      ...
      <div className="py-12 px-6 sm:px-10 md:px-16 xl:px-24">
        <h1 className=" text-[var(--text-primary)] font-medium text-3xl mb-4">Our Faculties</h1>
        ...
        <div className=" flex flex-col md:flex-row">
          {faculty.map((program, index) => {
            return (
              <div
                key={`faculty-${index}`}
                className=" space-y-6 p-4 bg-[var(--surface)] rounded-xl max-w-md"
              >
                <div>
                  <img src={FacultyTwo} alt="" className=" rounded-xl hover:bg-left-top bg-center bg-cover" />
                </div>
                  ...
                  {program.courses.map((course, i) => {
                    return (
                      ...
                      >
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#4e4e4e"><path d="m256-240-56-56 384-384H240v-80h480v480h-80v-344L256-240Z"/></svg>
                      </Link>
                    );
                  })}
                </div>
